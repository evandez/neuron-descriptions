...
class Serializable:
    ...
    recurse on. In particular, `children` should return map each recursively
    serializable property name to a string key representing its type. These
    unique type keys should be mapped back to python types in your override
    of the @classmethod `resolve`.
    ...
    without tying the payloads to this codebase. Indeed, you can open the
    payloads generated by `Serializable` anywhere because they're just
    dictionaries! This is especially useful for distributing research models,
    which are frequently shared across many different codebases in settings
    where the source is heavily, frequently, and indiscriminately modified.
    """
