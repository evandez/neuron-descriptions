...
class InstrumentedModel(torch.nn.Module):
    ...
    def retained_layer(self, aka=None, clear=False):
        ...
        # Special case: StyleGAN2 returns dicts.
    ...
    def _postprocess_forward(self, x, aka):
        ...
        if rule is not None:
            x = invoke_with_optional_args(rule,
                                          x,
                                          self,
                                          name=aka,
                                          **(self._editargs[aka]))
    ...
    def _hook_sequential(self):
        ...
        def new_forward(this,
            ...
            assert layer is None or (first_layer is None and
                                     last_layer is None)
            first_layer, last_layer = [
                str(layer)
                if layer is not None else str(d) if d is not None else None
...
def make_matching_tensor(valuedict, name, data):
    ...
    if len(v.shape) < len(data.shape):
        ...
        v = v.view((1,) + tuple(v.shape) + (1,) *
                   (len(data.shape) - len(v.shape) - 1))
        valuedict[name] = v
    return v
...
def subsequence(sequential,
    ...
    return hierarchical_subsequence(sequential,
                                    first=first,
                                    last=last,
                                    after=after,
                                    upto=upto,
...
def invoke_with_optional_args(fn, *args, **kwargs):
    ...
    if argspec.varkw is None:
        ...
        kwargs = {
            k: v for k, v in kwargs.items() if k in argspec.args or
